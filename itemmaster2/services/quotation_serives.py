...
class QuotationService:
    ...
    def process(self):
        ...
        with transaction.atomic():
            if self.status_text in ['Draft', 'Submit']:
                ...
                print("self.status_text", self.status_text)
    ...
    def update_status_instance(self): 
        self.status = CommanStatus.objects.filter(name=self.status_text, table="Quotations").first()
            ...
            if 'id' in self.data and self.data.get("id") != None: 
                ...
                if self.status_text == "Submit":
                    if 'parent_order' in self.data and self.data['parent_order'] is not None and Quotations.objects.filter(
                        ...
                        if parent.salesorder_2_set.exists():
                            status = parent.salesorder_2_set.first().status.name
    ...
    def validate_rate_sell_range(self):
        if self.status_text == 'Submit' and len(self.errors) == 0  :
    ...
    def update_tax_item_details(self):
        ...
        try:
            ...
            print("gst.gst_nature_type", gst.gst_nature_type)
            ...
            if is_specify_taxable:
                ...
                for item in item_details:
                    ...
                    if str(item.get("states", "")).lower() == str(self.state_of_company.address.state).lower():
                ...
                for charge in other_income_charge:
                    ...
                    if str(charge.get("states", "")).lower() == str(self.state_of_company.address.state).lower():
    ...
    def process_items(self):
        try:
            ...
            self.errors.append(f"Unexpeted error occurred is item Details save {str(e)}")
            return False
    ...
    def update_lead_reference(self, quotation):
        try:
            lead = quotation.lead_no if quotation.lead_no else None
            parent_id = self.data.get("parent_order")
      
            if not self.data.get("id") and lead:
    ...
    def create_or_update(self):
        ...
        if self.data.get("id"):
            ...
            serializer = QuotationsSerializer(quotation_instance, data=self.data, partial=True, context={'request': self.info.context})
        else:
            serializer = QuotationsSerializer(data=self.data, context={'request': self.info.context})
